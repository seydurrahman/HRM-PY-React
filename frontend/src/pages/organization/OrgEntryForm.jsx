...
export default function OrgEntryForm() {
  ...
  const loadSectionChilds = (section_id) => {
    ...
    api.get(`/org/subsections/?section_id=${section_id}`).then(res => setSubsections(res.data));
  ...
  const createItem = async (level) => {
    ...
    await api.post(`/org/${level}s/`, payload);
    ...
    if (level === "subsection" || level === "floor") loadSectionChilds(selected.section);
  ...
  return (
    <div className="p-6 space-y-6 max-w-xl">
      ...
      <input
        ...
        className="border p-2 w-full"
      />
      <button disabled={!selected.company}
      ...
      <input placeholder="New Division" disabled={!selected.unit}
        value={newName} onChange={(e) => setNewName(e.target.value)}
        onKeyDown={(e) => e.key === "Enter" && createItem("division")}
        className="border p-2 w-full"
      />
      <button disabled={!selected.unit}
        className="bg-blue-500 text-white px-4 py-2 disabled:bg-gray-400"
