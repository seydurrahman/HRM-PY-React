...
export default function FloorEntry() {
  ...
  const saveFloor = async () => {
    ...
    await api.post("/org/floors/", { name, section: selectedSection });
    setName("");
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <input placeholder="Floor Name" disabled={!selectedSection}
      ...
      <ul>
        {floors.map(f => (
          <li key={f.id} className="border p-2">{f.name}</li>
        ))}
      </ul>
    </div>
  );
