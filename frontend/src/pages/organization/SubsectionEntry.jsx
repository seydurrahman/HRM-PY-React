...
export default function SubsectionEntry() {
  ...
  const saveSubsection = async () => {
    ...
    await api.post("/org/subsections/", { name, section: selectedSection });
    setName("");
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <input placeholder="Subsection Name" className="border p-2 w-full"
        disabled={!selectedSection} value={name}
      ...
      <ul>
        {subsections.map(sub => (
          <li key={sub.id} className="border p-2">{sub.name}</li>
        ))}
      </ul>
    </div>
  );
